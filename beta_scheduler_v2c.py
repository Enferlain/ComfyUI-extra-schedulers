...
# Set to True to print sigma schedules for debugging/comparison with A1111
DEBUG_SIGMAS = True


def get_sigmas_like_kdiffusion(model_sampling, n):
    """
    Replicate k-diffusion's get_sigmas(n) behavior exactly.
    Uses the same floor/ceil/frac interpolation as k-diffusion's t_to_sigma().
    """
    # Get the model's full sigma table
    sigmas = model_sampling.sigmas.cpu().numpy()
    t_max = len(sigmas) - 1

    # k-diffusion: t values from t_max down to 0
    t = np.linspace(t_max, 0, n)

    # k-diffusion t_to_sigma logic: floor, ceil, frac interpolation
    ...
    w = t - np.floor(t)  # Fractional part (same as torch.frac)

    # Linear interpolation: (1 - w) * sigmas[low] + w * sigmas[high]
    result = (1 - w) * sigmas[low_idx] + w * sigmas[high_idx]

    # k-diffusion appends zero (append_zero function)
...
def get_beta_schedule_v2c(model_sampling, steps, alpha=0.6, beta=0.6):
    ...
    Now matches A1111's inner_model.get_sigmas() for base sigma generation.
    """
    # Get base sigma schedule using k-diffusion style interpolation (like A1111)
    base_sigmas = get_sigmas_like_kdiffusion(model_sampling, steps + 1)
    total_timesteps = len(base_sigmas) - 1

    if DEBUG_SIGMAS:
        raw_sigmas = model_sampling.sigmas.cpu().numpy()
        print(
            f"[Beta V2C] Raw sigma table: len={len(raw_sigmas)}, min={raw_sigmas.min():.6f}, max={raw_sigmas.max():.2f}"
        )
        print(f"[Beta V2C] Base sigmas (first 5): {base_sigmas[:5]}")
        print(f"[Beta V2C] Base sigmas (last 5): {base_sigmas[-5:]}")
    ...
    if DEBUG_SIGMAS:
        print(f"[Beta V2C] Alpha={alpha}, Beta={beta}, Steps={steps}")
        print(f"[Beta V2C] Result sigmas: {result_sigmas}")
...
def get_beta_schedule_v3(
    model_sampling, steps, alpha=0.6, beta=0.6, base_scheduler="normal"
):
    """
    ...
    base_sigmas = (
        comfy.samplers.calculate_sigmas(model_sampling, base_scheduler, high_res_steps)
        .cpu()
        .numpy()
    )
    total_timesteps = len(base_sigmas) - 1
    ...
    result_sigmas = (
        base_sigmas[idx_floor] * (1 - weights) + base_sigmas[idx_ceil] * weights
    )
    ...
    result_sigmas = np.minimum.accumulate(result_sigmas)
...
class BetaSchedulerV2CRaw:
    ...
    def get_sigmas(self, model, steps, alpha, beta, denoise):
        ...
        sigmas = get_beta_schedule_v2c_raw(model_sampling, total_steps, alpha, beta)
...
class BetaSchedulerV3:
    """
    Custom ComfyUI node for Beta Scheduler V3
    Production-ready Beta scheduler with vectorized operations and base scheduler selection.
    """
